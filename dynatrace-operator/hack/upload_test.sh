...
mpdev /scripts/install --deployer=$REGISTRY/$APP_NAME/deployer:$VERSION --parameters='{"name":"dynatrace-operator","namespace":"dynatrace","apiUrl":"<url>","apiToken":"<token>","paasToken":"<token>","infraMonitoring.enabled":true,"codeModules.enabled":true,"classicFullStack.enabled":false}'
